...
declareXlate l1 l2 xlate = do
  xlateType <- declareTypeAp xlate
  descendsA <- defineDescendA l1 l2 xlate
  pure $ xlateType : descendsA
...
declareTypeAp :: XlateDef -> Q Dec
declareTypeAp x = do
  TH.addModFinalizer $ TH.putDoc (TH.DeclDoc xlateName) $ unlines
    [ "This type is used to parameterize the nanopass-generated translation functions @descend*A@."
    , "It has members for:"
    , ""
...
defineDescendA l1 l2 xdef = do
  fmap concat . forM xdef.xlateSyncats $ \XlateSyncatDef{syncatName} -> do
    ...
    TH.addModFinalizer $ TH.putDoc (TH.DeclDoc funName) $ unlines
      ...
      , unwords
        [ "It is parameterized by an t'XlateA', which"
