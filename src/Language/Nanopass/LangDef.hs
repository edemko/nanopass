...
defineLang :: Language -> Define [Dec]
defineLang l = do
  ...
  if not (null duplicateParams)
    ...
    else modify $ \st -> st{ langTyvars = TH.mkName . fromLowName <$> l.langParams }
  -- initialize syncatNames
  forM_ l.syncats $ \syncat -> do
    knownNames <- gets syncatNames
    case Map.lookup syncat.syncatName knownNames of
      Nothing -> modify $ \st ->
        st{syncatNames = Map.insert syncat.syncatName (TH.mkName $ fromUpName syncat.syncatName) knownNames}
      Just _ -> fail $ concat [ "in a nanopass language definition: "
                              , "duplicate syntactic category (terminal/nonterminal) name "
                              , fromUpName syncat.syncatName
                              ]
  ...
  syncatTypeDecs <- forM (Map.elems l.syncats) $ \syn -> do
    let syncatName = TH.mkName $ fromUpName syn.syncatName
    M.lift $ TH.addModFinalizer $ TH.putDoc (TH.DeclDoc syncatName) $
      "This type is a syntactic category of the t'" ++ fromUpDotName l.langName ++ "' language."
    prodCtors <- defineProduction `mapM` Map.elems syn.productions
    pure $ TH.DataD [] syncatName params Nothing
            prodCtors
            []
  pure $ langInfo : syncatTypeDecs

defineLanginfo :: Language -> Define Dec
defineLanginfo l = do
  syncatNames <- gets $ Map.toAscList . syncatNames
  ctors <- forM syncatNames $ \(syncatName, _) -> do
    let ctorName = TH.mkName $ fromUpDotName l.langName ++ "_" ++ fromUpName syncatName
  ...
  let thName = TH.mkName $ fromUpDotName l.langName
  M.lift $ TH.addModFinalizer $ TH.putDoc (TH.DeclDoc thName) $ concat
    ...
    , case (l.baseDefdLang, l.originalProgram) of
      (Just l0, Just origProg) -> unlines
        [ ""
        , "This language was generated based on the langauge t'" ++ show l0.langNameTH ++ "'"
...
defineProduction :: Production -> Define TH.Con
defineProduction production = do
  fields <- defineSubterm `mapM` production.subterms
  pure $ TH.NormalC (TH.mkName $ fromUpName production.prodName) fields
...
reifyLang langName = do
  ...
  syncatTypeList <- forM thSyncats $ \(syncatName, syncatNameTH, paramNames, thCtors) -> do
    ...
    case duplicatePNames of
      [] -> pure Syncat
        { syncatName
        , syncatNameTH
        , productions = Map.fromList (ctorList <&> \ctor -> (ctor.prodName, ctor))
        }
  ...
  let syncatTypes = syncatTypeList <&> \t -> (t.syncatName, t)
  ...
  langParams <- do
    ...
    rawTvs <- fromMaybe [] <$> foldM f Nothing thSyncats
  ...
  pure $ Language
    ...
    , langNameTH
    , langParams
    , syncats = Map.fromList syncatTypes
  ...
  fixup = reverse . loop . reverse . show
    ...
    loop (c:rest)
      ...
      | '0' <= c && c <= '9' = loop rest
    loop other = other
  decodeCtor :: [UpName] -> [TH.Name] -> TH.Con -> Q Production
  decodeCtor sNames paramNames (TH.NormalC prodNameTH thSubterms) = do
    ...
    pure $ Production{prodName,prodNameTH,subterms}
  ...
  findRecursiveType (TH.NormalC thTypePtr []) = do
    ...
    TH.reify typePtr >>= \case
      TH.TyConI (TH.DataD [] syncatNameTH thParams _ ctors _) -> do
        syncatName <- case toUpName $ TH.nameBase syncatNameTH of
          Just x -> pure x
          Nothing -> fail $ "corrupt language has illegal syntactic category name: " ++ show syncatNameTH
        let thParamNames = thParams <&> \case { TH.PlainTV it _ -> it ; TH.KindedTV it _ _ -> it }
        pure (syncatName, syncatNameTH, thParamNames, ctors)
  ...
  doSyncat l (ModSyncat sName prodMods) = case Map.lookup sName l.syncats of
    ...
    doProds s (DelProd pName) = case Map.lookup pName s.productions of
      ...
      Nothing -> fail $ concat
        ...
        , fromUpName sName ++ " from " ++ show s.syncatName ++ " in " ++ show l.langNameTH
        ]
...
extendLang l lMods = do
  ...
  doSyncat gMods syncat0 = do
    let prodList
          ...
          ++ catAddProd syncat0.syncatName gMods
    pure Syncat
      { syncatName = syncat0.syncatName
      , syncatNameTH = TH.mkName $ fromUpName syncat0.syncatName
      , productions = Map.fromList $ prodList <&> \prod -> (prod.prodName, prod)
      }
